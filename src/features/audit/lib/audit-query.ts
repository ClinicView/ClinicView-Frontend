...
export function parseAuditFilterDraft(draft: AuditFilterDraft): ParsedAuditFilters {
  ...
  if (action && (!ACTION_PATTERN.test(action) || action.length > 64)) {
    errors.action = 'Usa solo letras, números y guiones bajos (máximo 64 caracteres).';
  }

  if (resourceType && (!ACTION_PATTERN.test(resourceType) || resourceType.length > 32)) {
    errors.resourceType = 'Usa solo letras, números y guiones bajos (máximo 32 caracteres).';
  }
  ...
  const from = draft.fromDate ? limaDayBoundary(draft.fromDate, 'start') : undefined;
  const to = draft.toDate ? limaDayBoundary(draft.toDate, 'end') : undefined;
  if (from && to && Date.parse(from) > Date.parse(to)) {
    errors.dateRange = 'La fecha inicial no puede ser posterior a la fecha final.';
  }
  ...
  return {
    filters: {
      ...
      ...(draft.resourceId.trim() ? { resourceId: draft.resourceId.trim() } : {}),
...
export function buildAuditEventsQuery(filters: AuditFilters, cursor?: string | null): string {
  const params = new URLSearchParams();

  if (cursor) params.set('cursor', cursor);
  const entries = Object.entries(filters) as Array<[keyof AuditFilters, AuditFilters[keyof AuditFilters]]>;
...
export function countActiveAuditFilters(filters: AuditFilters): number {
  return Object.entries(filters).filter(([key, value]) => (
    key !== 'limit' && value !== undefined && value !== ''
  )).length;
}
