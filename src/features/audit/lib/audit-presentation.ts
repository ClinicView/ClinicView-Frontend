...
export const AUDIT_OUTCOME_PRESENTATION: Record<AuditOutcome, {
  label: string;
  description: string;
}> = {
  SUCCESS: { label: 'Correcto', description: 'La operación terminó correctamente.' },
  DENIED: { label: 'Denegado', description: 'El sistema rechazó el acceso solicitado.' },
  FAILED: { label: 'Fallido', description: 'La operación terminó con un error.' },
};
...
export function getAuditResourceLabel(resourceType?: string | null): string {
  ...
  return RESOURCE_LABELS[resourceType] ?? resourceType.replaceAll('_', ' ').toLowerCase();
}

export function formatAuditDate(iso: string): string {
  const date = new Date(iso);
  return Number.isNaN(date.getTime()) ? 'Fecha no disponible' : limaDateTimeFormatter.format(date);
}
