...
export const RECORD_TYPE_DEFINITIONS: Readonly<Record<RecordType, RecordTypeDefinition>> = {
    ...
    sections: [
      {
        ...
        fields: [
          ...
          { key: 'careInstructions', label: 'Orientación y signos de alarma explicados', kind: 'textarea', maxLength: 2000, description: 'Consigna únicamente la orientación efectivamente brindada o documentada; no se genera automáticamente.' },
  ...
  EVOLUTION: {
    ...
    sections: [{
      id: 'evolution-note', title: 'Evolución clínica', fields: [
        ...
        { key: 'disposition', label: 'Destino o conducta al finalizar', kind: 'textarea', maxLength: 1000 },
        { key: 'followUp', label: 'Seguimiento', kind: 'textarea', maxLength: 2000 },
      ],
    }],
  },
  LAB_RESULT: {
    ...
    sections: [
      { id: 'lab-study', title: 'Datos del estudio', fields: [
        ...
        { key: 'methodology', label: 'Método analítico', kind: 'text', maxLength: 1000 },
        { key: 'sampleCondition', label: 'Condición de la muestra', kind: 'text', maxLength: 1000 },
        { key: 'criticalResultCommunication', label: 'Comunicación de resultado crítico', kind: 'textarea', maxLength: 2000, description: 'Si consta, registra a quién se comunicó, cuándo y por qué medio. Este campo no envía una alerta.' },
  ...
  PRESCRIPTION: {
    ...
    sections: [
      { id: 'prescription-context', title: 'Indicación y vigencia', fields: [
        ...
        { key: 'safetyReview', label: 'Verificación documentada de seguridad', kind: 'textarea', maxLength: 2000, description: 'Alergias, interacciones u otras verificaciones realizadas. No sustituye una evaluación profesional ni comprueba interacciones automáticamente.' },
  ...
  PROCEDURE: {
    ...
    sections: [{ id: 'procedure-detail', title: 'Detalle del procedimiento', fields: [
      ...
      { key: 'materials', label: 'Materiales o dispositivos utilizados', kind: 'textarea', maxLength: 2000 },
      { key: 'specimenDestination', label: 'Muestras obtenidas y destino', kind: 'textarea', maxLength: 1000 },
  ...
  THERAPY_NOTE: {
    ...
    sections: [
      { id: 'therapy-session', title: 'Sesión terapéutica', fields: [
        ...
        { key: 'sessionDurationMinutes', label: 'Duración de la sesión (minutos)', kind: 'number', min: 1, max: 1440, step: 1, inputMode: 'numeric' },
        { key: 'tolerance', label: 'Tolerancia y limitaciones observadas', kind: 'textarea', maxLength: 1000 },
  ...
  OTHER: {
    ...
    sections: [{ id: 'other-detail', title: 'Contenido clínico', fields: [
      ...
      { key: 'recipient', label: 'Destinatario (si corresponde)', kind: 'text', maxLength: 300 },
      { key: 'purpose', label: 'Finalidad del documento', kind: 'textarea', maxLength: 1000 },
